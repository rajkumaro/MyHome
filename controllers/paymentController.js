...
const processPayment = async (req, res) => {
  try {
    ...
    if (paymentIntent.status !== 'succeeded') {
      return res
        .status(400)
        .json({ error: `Payment not succeeded. Current status: ${paymentIntent.status}` });
    }
    ...
    await invoiceService.createInvoice(bookingId, transaction._id, {
...
const addPaymentMethod = async (req, res) => {
  try {
    ...
    const hasDefault = await PaymentMethod.findOne({ userId: req.user.id, isDefault: true });
...
const deletePaymentMethod = async (req, res) => {
  try {
    ...
    try {
      ...
      console.error('Stripe detach error (non-fatal):', stripeErr.message);
    }
...
const setDefaultPaymentMethod = async (req, res) => {
  try {
    ...
    const method = await PaymentMethod.findOneAndUpdate(
      { userId: req.user.id, stripePaymentMethodId: paymentMethodId },
      { isDefault: true },
      { new: true }
    );
